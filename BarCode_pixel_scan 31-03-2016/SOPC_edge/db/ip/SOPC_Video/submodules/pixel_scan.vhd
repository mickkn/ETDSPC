...
BEGIN
	...
	PROCESS(avs_s1_read) --sensitivity list
		...
		VARIABLE start_bits		: INTEGER := 1;
		VARIABLE ean_counter	: INTEGER := 0;

	
		BEGIN
			--IF rising_edge(avs_s1_clk) THEN
				--IF (avs_s1_read = '1') THEN	--send new data	
				IF avs_s1_read = '0' THEN
					IF (mm_data_ready = '1') THEN
					
						IF (ean_counter = 13) THEN
							ean_counter := 0;
						END IF;
						
						IF (start_bits = 1) THEN
							avs_s1_readdata <= "10000000";
							start_bits := 0;
						ELSE
							avs_s1_readdata <= "0000" & ean_number_debug(ean_counter);
